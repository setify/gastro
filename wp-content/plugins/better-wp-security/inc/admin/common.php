		...
		function getrules() {
			...
			if ( $bwpsoptions['st_ht_query'] == 1 ) {
				...
				if ( $bwpsserver == 'apache' || $bwpsserver == 'litespeed' ) {
					...
					$rules .= 
						...
						"\tif (\$postArgs ~* \"\\.\\./\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"(bash|git|hg|log|svn|swp|cvs)\") { set \$susquery 1; }" .PHP_EOL .
						"\tif (\$postArgs ~* \"etc/passwd\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"boot.ini\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"ftp:\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"http:\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"https:\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"(<|%3C).*script.*(>|%3E)\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"mosConfig_[a-zA-Z_]{1,21}(=|%3D)\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"base64_encode\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"(%24&x)\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"(\\[|\\]|\\(|\\)|<|>|ê|\\\"|;|\?|\*|=$)\"){ set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"(&#x22;|&#x27;|&#x3C;|&#x3E;|&#x5C;|&#x7B;|&#x7C;|%24&x)\"){ set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"(%0|%A|%B|%C|%D|%E|%F|127.0)\") { set \$susquery 1; }" . PHP_EOL .
						"\tif (\$postArgs ~* \"(globals|encode|localhost|loopback)\") { set \$susquery 1; }" .PHP_EOL .
						"\tif (\$postArgs ~* \"(request|select|insert|concat|union|declare)\") { set \$susquery 1; }" . PHP_EOL;
			...
			if ( $bwpsserver == 'nginx' ) {
				...
				$rules .= 
					...
					"\t}" . PHP_EOL . 
					"\tif (\$postArgs !~ \"^loggedout=true\") { set \$susquery 3\$susquery; }" . PHP_EOL;
					"\tif (\$postArgs !~ \"^action=rp\") { set \$susquery 4\$susquery; }" . PHP_EOL;
			...
			if ( $bwpsoptions['hb_enabled'] == 1 ) {
				...
				if ( $bwpsserver == 'apache' || $bwpsserver == 'litespeed' ) {
					...
					$rules .= 
						...
						"\tif (\$postArgs !~ \"^action=logout\") { set \$rule_3 8\$rule_3; }" . PHP_EOL .
						"\tif (\$postArgs !~ \"^" . $key . "\") { set \$rule_3 9\$rule_3; }" . PHP_EOL .
						"\tif (\$postArgs !~ \"^action=rp\") { set \$rule_3 0\$rule_3; }" . PHP_EOL .
						"\tif (\$postArgs !~ \"^action=register\") { set \$rule_3 a\$rule_3; }" . PHP_EOL .
						"\tif (\$postArgs !~ \"^action=postpass\") { set \$rule_3 b\$rule_3; }" . PHP_EOL .
